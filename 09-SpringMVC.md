...
# 一、SpringMVC
![](./pictures/springMVC/springMVCContent.png)
## 1、什么是MVC
...
<servlet-mapping>
    ...
    <!--
        ...
        但是/不能匹配.jsp请求路径的请求（jsp 有自己的处理 Servlet，若被匹配则得不到处理）
    -->
...
```
> 注意：所有的页面跳转和访问操作和方法名并没用关系，方法名仅仅起到见名知意的作用。
...
```
> 注意：访问路径分为**浏览器访问路径**和**服务器访问路径**。
浏览器访问路径默认从 localhost:8080/ 开始，没用正确的 web 项目访问上下文路径。这里使用 Thymeleaf 表达式可以自动补齐正确路径，方便访问和维护。
...
**启动服务器后多做了什么操作**
| **操作**         | **Spring MVC 应用**            | **纯 Servlet/JSP 应用**       |
|------------------|------------------------------------|-----------------------------|
| 容器初始化        | 解析 web.xml 中的配置加载 Spring 容器 + ServletContext           | 仅加载 ServletContext            |
| 请求分发          | 由 `DispatcherServlet` 统一处理             | 由多个 Servlet 各自处理           |
| 静态资源          | 需显式配置或交由默认 Servlet                | 自动由 Tomcat 默认 Servlet 处理   |
| 注解解析          | 解析 `@Controller`、`@RequestMapping` 等    | 无                                |
| 视图渲染          | 通过 `ViewResolver` 动态渲染                | 手动 forward 或 write HTML       |
...
```java
@RequestMapping(value = "/testServletAPI3")
// 只要形参一致 dispatcher 自动赋值
public String testServletAPI3(String username, String password, @RequestParam("hobbyby") String[] hobbys,
                            @RequestHeader("host") String host,
                            @CookieValue(value = "JSESSIONID", required = false) String JSESSIONID
) {
    System.out.println("username=" + username + ", password=" + password + ", hobby=" + Arrays.toString(hobbys) + ", host=" + host + ", JSESSIONID=" + JSESSIONID);
    return "success";
}
```
...
![](./pictures/springMVC/postmanJson.png)

@RequestBody可以接收 Json 格式数据自动反序列化，并为形参的 POJO 形参赋值：
```java
...
文件上传和下载使用 [SSM-SpringBoot](./SSM-SpringBoot.md) 整合中的方式，更规范、更简便。
## ~~1、文件下载~~
