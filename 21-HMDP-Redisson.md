...
public Result seckillVoucher(Long voucherId) {
        ...
        // 细节1：必须确保锁包含整个事务，更新数据库之后才能释放锁
        boolean isLock = lock.tryLock();
        if (!isLock) {
            return Result.fail("活动期间每个用户只能下一单！");
        }

        try {
            // 细节2：事务失效问题：因为事务是基于代理的，而this调用方法不会走代理所以事务会失效
